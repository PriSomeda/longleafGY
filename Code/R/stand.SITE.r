...
stand.SITE  <-  function(HDOM=NA, SI=NA, AGE=NA){
  ...
  if(is.na(HDOM)==FALSE && is.na(SI)==FALSE && is.na(AGE)==FALSE){
    print("Warning - No parameteres to estimated. You already have all information.")
  }

  # 2. If HDOM is missing
  else if(is.na(HDOM)==TRUE && is.na(SI)==FALSE && is.na(AGE)==FALSE){
    HDOM <- SI*((1-exp(a1*AGE))/(1-exp(a1*50)))^a2
  }

  # 3. If SI is missing
  else if(is.na(HDOM)==FALSE && is.na(SI)==TRUE && is.na(AGE)==FALSE){
    SI <- (HDOM/((1-exp(a1*AGE))/(1-exp(a1*50)))^a2)
  }

  # 4. If AGE is missing
  else if(is.na(HDOM)==FALSE && is.na(SI)==FALSE && is.na(AGE)==TRUE){
    min.error <- 10000
  ...
  else if(is.na(HDOM)==TRUE && is.na(SI)==TRUE && is.na(AGE)==TRUE |
     is.na(HDOM)==FALSE && is.na(SI)==TRUE && is.na(AGE)==TRUE |
     is.na(HDOM)==TRUE && is.na(SI)==FALSE && is.na(AGE)==TRUE |
     is.na(HDOM)==TRUE && is.na(SI)==TRUE && is.na(AGE)==FALSE){
